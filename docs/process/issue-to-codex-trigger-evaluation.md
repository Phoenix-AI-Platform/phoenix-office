...
The runtime probe is intentionally local and non-invoking. It requires the installed CLI to advertise non-interactive `codex exec`, stdin or `-` prompt input, `--ephemeral`, `--sandbox`, `--cd` or `-C`, `--json`, `--output-last-message` or `-o`, and an explicit per-run budget-style option before reporting local CLI readiness. It does not submit prompts, run Codex tasks, authenticate, access GitHub, access the network, inspect live branches or PRs, write files, dispatch workflows, or mutate repository state. Even if all local capabilities are detected, `pilot_ready` remains `false`; authentication, runner access, enforceable per-run budget ceilings, cancellation behavior, GitHub permissions, duplicate PR detection, branch-collision detection, actual Codex availability, final CI, and assistant review remain external checks.

## Supervised Pilot External Controls Checklist

This checklist records evidence only. Completing it does not authorize Codex invocation, branch creation, PR creation, approval, merge, workflow dispatch, proposal/DOCX generation, orchestration execution, or background work. `pilot_ready` remains `false` until every external control is verified and a separate reviewed issue explicitly authorizes a supervised pilot. Missing, stale, ambiguous, or unverifiable evidence blocks the pilot.

| External control | Evidence required | Acceptable result | Failure or unknown result | Blocks pilot | Responsible reviewer |
| --- | --- | --- | --- | --- | --- |
| Codex authentication and runner access | Operator-captured proof that the selected runner can authenticate to the intended Codex account without exposing credentials, tokens, home directories, or config contents. | Authentication is available for the selected runner and account, and logs contain no secrets or private customer data. | Authentication cannot be proven, evidence is stale, logs expose sensitive data, or the runner identity is unclear. | Yes | Human operator |
| Enforceable per-run budget or usage ceiling | Platform documentation or runner configuration showing a per-run token, cost, or usage ceiling for the supervised invocation. | A per-run ceiling is configured before invocation and failure behavior is fail-closed when the ceiling cannot be enforced. | Only account-wide usage limits exist, the ceiling is advisory/reporting-only, enforcement is unclear, or no evidence is provided. | Yes | Human operator and assistant reviewer |
| Operator cancellation and timeout behavior | Evidence from the selected runner showing how an operator cancels an active run and how the hard timeout is enforced. | Cancellation stops further work immediately, and the run has a hard maximum runtime with no retries, queueing, scheduling, or background continuation. | Cancellation behavior is unknown, cancellation allows follow-up activity, timeout is missing, or retries/background work can continue. | Yes | Human operator |
| GitHub branch creation permission | Permission review for the credential or app identity that would create the supervised branch. | The identity can create exactly the intended branch class and does not have broader repository administration permissions. | Branch permission is missing, overly broad, tied to an unclear identity, or cannot be audited. | Yes | Human operator |
| GitHub pull-request creation permission | Permission review for the credential or app identity that would open the supervised PR. | The identity can open one PR for the selected handoff and cannot approve, merge, mutate labels, comment, dispatch workflows, or update branches unless a later reviewed PR explicitly authorizes it. | PR permission is missing, too broad, allows approval/merge side effects, or cannot be audited. | Yes | Human operator and assistant reviewer |
| Confirmation that Codex cannot approve or merge | Evidence from permissions, branch protection, or workflow policy showing Codex cannot approve or merge its own PR. | Codex-created work requires separate assistant review and normal CI before any routine merge; Codex itself cannot approve or merge. | Codex can approve, merge, bypass review, bypass CI, or the authority boundary is unclear. | Yes | Assistant reviewer |
| Duplicate active-PR detection | Dry-run evidence showing the selected issue, handoff id, expected branch, and expected PR title are checked against active PRs before branch creation. | Existing active PRs for the same issue or handoff block invocation before branch creation. | Duplicate detection is missing, manual-only, stale, ambiguous, or runs after mutation. | Yes | Assistant reviewer |
| Branch-collision detection | Dry-run evidence showing the expected branch name is checked before branch creation. | Existing branch collisions block invocation before branch creation. | Branch-collision detection is missing, stale, ambiguous, or runs after mutation. | Yes | Assistant reviewer |
| Actual Codex availability during a supervised task | A non-mutating operational check or reviewed dry-run result showing the selected runner is available at task time. | Codex is available for the selected runner and task window without requiring prompt submission or task execution as proof. | Availability is assumed from installation, stale, unavailable, or requires executing an unapproved task to prove. | Yes | Human operator |
| Final CI requirement | Repository branch-protection or process evidence naming the required checks for the PR class. | The supervised PR cannot be considered for routine merge until final CI and required body guards pass on the PR head. | CI requirements are unclear, failing, pending without review, bypassable, or checked on the wrong commit. | Yes | Assistant reviewer |
| Assistant architecture review requirement | Review process evidence showing a separate assistant architecture review remains required after the PR opens. | Assistant review evaluates scope, changed files, safety boundaries, and CI before any merge recommendation. | Review is skipped, performed by the same Codex worker as self-approval, or treated as automatic approval. | Yes | Assistant reviewer |
...
Exit zero means only that the package is structurally valid and all eleven required external controls are marked `verified` with safe opaque evidence references and expected reviewer roles. It does not prove the referenced evidence is truthful, current, enforceable, sufficient, or reviewed. The output must keep `pilot_ready: false`, `invocation_authorized: false`, `invocation_performed: false`, `github_access_performed: false`, `network_access_performed: false`, and `mutation_performed: false`.
