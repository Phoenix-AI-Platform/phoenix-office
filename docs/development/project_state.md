...
#170 docs/process: validate self-contained docs-only auto-merge label path (adds a tiny docs-only pilot validation note for testing that one human-applied `phoenix-automerge-docs` label triggers dry-run, wakes the pilot, and merges without a manual dry-run rerun)
```
