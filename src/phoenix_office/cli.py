...
def build_parser() -> argparse.ArgumentParser:
    ...
    records_proposal_build_parser = records_subparsers.add_parser(
        "proposal-build",
        help="Build ProposalInput JSON and DOCX from stored records and proposal details",
    )
    records_proposal_build_parser.add_argument(
        "customer_id",
        help="Customer ID to use",
    )
    records_proposal_build_parser.add_argument(
    ...
    records_proposal_build_parser.add_argument(
        ...
        type=Path,
        help="Path for normalized ProposalInput JSON",
    )
    records_proposal_build_parser.add_argument(
        "output_docx",
        type=Path,
        help="Path for generated proposal DOCX",
    )
    records_proposal_build_parser.add_argument(
        "--db",
        type=Path,
    ...
    records_proposal_build_parser.set_defaults(func=build_record_proposal)
...
def build_record_proposal(args: argparse.Namespace) -> int:
    ...
    try:
        details = record_proposal_details_from_file(details_path)
    except (ValueError, ValidationError):
        print(f"Error: invalid RecordProposalDetails JSON: {details_path}", file=sys.stderr)
        return 1
    except Exception:  # noqa: BLE001 - sanitize details-loading errors at CLI boundary.
        print(f"Error: failed to load RecordProposalDetails JSON: {details_path}", file=sys.stderr)
        return 1
